...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert lt(sigmoid(a), 0.0) == 0.0
    assert lt(1.0, sigmoid(a)) == 0.0
    assert_close(1 - sigmoid(a), sigmoid(-a))
    assert_close(sigmoid(0), 0.5)
    assert lt(sigmoid(a + 1.0), sigmoid(a)) == 0.0
...
def test_other(x: float) -> None:
    ...
    """
    if lt(0, x) == 1.0:
        assert lt(relu(x), relu(x + 1.0)) == 1.0
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    sum_list = addLists(ls1, ls2)
    for i in range(len(sum_list)):
        assert_close(sum_list[i], add(ls1[i], ls2[i]))
